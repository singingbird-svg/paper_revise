...
we develop a decomposition-based semi-distributed task allocation and planning
framework ($\mathrm{D}^{2}\mathrm{TAPR}$). The framework is semi-distributed because collaborative subtasks use leader-based coordination. The global task specified in linear temporal logic
...
subtasks are reassigned through a free-market-based mechanism,
which uses a designated free-market region for information
exchange and subtask redistribution. A patrol mechanism is
triggered when prerequisite completion remains unconfirmed beyond the estimated completion time, gathering information to prevent
...
Under LTL constraints, existing task allocation and planning methods are typically categorized into top-down and bottom-up approaches. In top-down methods, a global LTL formula specifies the overall mission, and a centralized planner decomposes this specification into robot-level plans. A common approach constructs a product automaton from a global LTL-derived nondeterministic B"uchi automaton (NBA) and robot discrete transition systems, and then solves the resulting graph problem\cite{smith2010optimal,ulusoy2013optimality}. However, state explosion caused by increased robot numbers, larger environments, and more complex tasks limits this method scalability in large systems.
...
Accepting runs can be represented in a prefix鈥搒uffix structure, where the prefix $w_{\text{pre}}$ runs from an initial state to an accepting state, and the suffix $w_{\text{suf}}$ forms a cycle containing at least one accepting state.
...
% 			$S_k(\tau_n)$ & 聽the set聽of聽states聽of聽robots聽received聽by  robot $r_k$ at time $\tau_n$  \\\hline
...
ii) Over-assignment: If the number of robots committed to $a_j$ exceeds the required cardinality $\mathrm{LR}(a_j)$, robot $r_k$ ranks all committed robots by their remaining travel time to the target region (smallest $t_a^m$ first) and keeps only the first $\mathrm{LR}(a_j)$ robots as valid executors of $a_j$. If multiple robots have the same $t_a^m$, the tie is broken by robot ID in ascending order. If $r_k$ is not selected after this filtering step, it immediately stops the current commitment and replans.
...
iii) robots expected to participate in a collaborative subtask do not reach the target region within the collaboration waiting time or the subsequent recruitment timeout.
...
%This section evaluates the proposed algorithm via simulations and physical experiments. LTL2BA \cite{10.1007/3-540-44585-4_6} is used to convert LTL formulas into NBA. The algorithm is implemented in Python 3 and tested on an 11th Gen Intel庐 Core鈩?i7-11390H with 16 GB RAM. Physical experiments run on Ubuntu 18.04 with ROS.
The algorithm is evaluated through simulations and physical experiments. LTL formulas are converted to NBA using LTL2BA \cite{10.1007/3-540-44585-4_6}. Simulations run in Python 3  on an 11th Gen Intel庐 Core鈩?i7-11390H with 16 GB RAM. Physical experiments use Ubuntu 18.04 with ROS. 
...
iv) Test 4 examines $\mathrm{D^2TAPR}$鈥檚 adaptability to dynamic robot changes (e.g., robots joining, or temporarily becoming unavailable during execution). We evaluate whether the framework can maintain feasible allocation updates and continue task progress without global replanning. 
...
\begin{figure}[thpb]
      \centering
        ...
        \caption{
%        	
Simulation environment for Tests. The 30脳15 simulation environment contains 12 target regions $l_1$$\sim$$l_{12}$ and 12 robots $r_1$$\sim$$r_{12}$ . 
%30脳15 environment contains 12 target regions ($l_1$ to $l_{12}$), 12 robots of 3 types (blue, red, yellow dots), and obstacles (black squares).
        	}
    \label{workspace}
   \end{figure}
...
%Notably, at $D = 5$ m, disabling the patrolling mechanism prevents robots from receiving $q_1$鈥檚 completion message, causing the task to remain unfinished.
...
%%	{Task execution in Case 1 when $D=15$ m. The X-axis shows time, and the Y-axis lists subtasks. The wider block marks the robot鈥檚 current task; the narrower block in the subtask鈥檚 color means the robot is waiting for others. Gray lines indicate there is no assigned subtask and the robot is not in the free market.}
%	\label{com15}
%\end{figure}
